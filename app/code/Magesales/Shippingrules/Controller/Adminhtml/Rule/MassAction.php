...
class MassAction extends \Magesales\Shippingrules\Controller\Adminhtml\Rule
{
    public function execute()
    {
        ...
        if ($ids && in_array($action, ['activate', 'inactivate', 'delete'])) {
            try {
                ...
                switch($action) {
                    case 'delete':
                        $collection = $this->_objectManager->create('Magesales\Shippingrules\Model\Resource\Rule\Collection');
                ...
                if($status > -1) {
                    $this->_objectManager->create('Magesales\Shippingrules\Model\Rule')->massChangeStatus($ids, $status);
                }
