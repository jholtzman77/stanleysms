...
class Ups extends \Plumrocket\ShippingTracking\Model\AbstractService
{
    ...
    public function parseTrackInfo($info)
    {
        ...
        if (isset($info['TrackResponse']['Shipment']['Package']['Activity'])) {
            ...
            if(isset($info['Status']) && isset($info['Date']) && isset($info['Time'])){
    ...
    private function dataSorting($activity)
    {
        ...
        $result['status'] = isset($activity['Status']['Description'])
            ? $activity['Status']['Description'] : '';

        return $result;
    }

    public function explodeDate(string $orderDate)
    {
     ...
     */
    public function explodeTime(string $orderTime)
    {
        $currentTime = substr($orderTime, 0, 2) . ':' . substr($orderTime, 2, 2);
        $orderTime = date('h:i A', strtotime($currentTime));
